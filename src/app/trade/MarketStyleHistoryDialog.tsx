...
function buildStylesByDate(items: MarketStyleSnapshot[] | undefined) {
  const map = new Map<string, MarketStyleSnapshot>();
...
function StyleDayCell({
  ...
  const visual =
    item && MARKET_STYLE_VISUAL[item.market_style]
  ...
  return (
    <Tooltip title={<MarketStyleTooltipContent item={item} detail="day" />} variant="solid">
      {content}
    </Tooltip>
  );
}

export default function MarketStyleHistoryDialog({ open, onClose }: Props) {
  ...
  const stylesByDate = useMemo(() => buildStylesByDate(data as MarketStyleSnapshot[]), [data]);
